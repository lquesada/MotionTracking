...
void capturaTextura() {
  ...
  IplImage *newimage2 = cvCreateImage(cvSize(RESOLUTION_W_SHOW,RESOLUTION_H_SHOW), IPL_DEPTH_8U, 3);
  cvResize(frame,newimage2,CV_INTER_CUBIC);
  frames = Convert_OpenCV_TO_OSG_IMAGE(newimage2);
  osg::ref_ptr<osg::Image> aux = Convert_OpenCV_TO_OSG_IMAGE(newimage);
  mpcambios.lock();
  ultima = nueva;
  nueva = aux;
  mpcambios.unlock();
...
int main( int, char** )
{
  ...
  ref_ptr<Group> grupomain = new Group;
  ref_ptr<MatrixTransform> mTmain = new MatrixTransform;
  mTmain->addChild(geode.get());
  mTmain->setMatrix(CTransform());
  grupomain->addChild(mTmain);
  osg::DisplaySettings * ds = osg::DisplaySettings::instance();
  // printf("Activando antialiasing\n");
  ds->setNumMultiSamples(4);
  viewer->realize();
  if (!viewer->isRealized()) {
    viewer = new Viewer;
    // printf("Desactivando antialiasing\n");
    ds->setNumMultiSamples(0);
    viewer->realize();
  }
  viewer->setSceneData (grupomain.get());
  init_viewer(*viewer);
  while (!viewer->done()) {
    if (acumtime()) {
      timer();
